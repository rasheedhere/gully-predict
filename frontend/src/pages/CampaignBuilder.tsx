...
import {
  ...
  Play, Lock, FileEdit, BarChart2, Users, Copy, Star, Trophy, ArrowDownAZ, MapPin, Search, X
} from 'lucide-react';
...
export function CampaignForm({ campaignId }: { campaignId?: string }) {
  ...
  const [maxPowerups, setMaxPowerups] = useState<number | null>(null);
  const [penalty, setPenalty] = useState(0);
  const [leagueId, setLeagueId] = useState<string | null>(null);
  const [targetMatchIds, setTargetMatchIds] = useState<string[]>([]);
  ...
  const { data: matches } = useMatches(tournamentId || undefined);
  ...
  useEffect(() => {
    if (existing) {
      ...
      setMaxPowerups(existing.max_powerups ?? null);
      setPenalty(existing.non_participation_penalty ?? 0);
      setLeagueId(existing.league_id);
      setTargetMatchIds(existing.target_match_ids || []);
  ...
  const selectedMatches = useMemo(() => matches?.filter(m => targetMatchIds.includes(m.id)) || [], [matches, targetMatchIds]);
  const unselectedMatches = useMemo(() => matches?.filter(m => !targetMatchIds.includes(m.id)) || [], [matches, targetMatchIds]);
  const filteredMatches = useMemo(() => {
    if (!matchSearch.trim()) return unselectedMatches;
    const lowerSearch = matchSearch.toLowerCase();
    return unselectedMatches.filter(m => 
      m.team1.toLowerCase().includes(lowerSearch) || 
      m.team2.toLowerCase().includes(lowerSearch) ||
      m.id.toLowerCase().includes(lowerSearch) ||
      (m.venue && m.venue.toLowerCase().includes(lowerSearch))
    );
  }, [unselectedMatches, matchSearch]);
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      max_powerups: maxPowerups,
      non_participation_penalty: penalty,
      league_id: leagueId,
      target_match_ids: targetMatchIds.length > 0 ? targetMatchIds : undefined,
      ...
      <div className="glass-panel p-6 space-y-4 border-t-2 border-t-ipl-gold">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-gray-600 text-[10px] font-display mt-1">Penalty for no response.</p>
          </div>
          {type === 'match' && (
            <div>
              <label className="text-gray-500 text-[10px] font-display uppercase tracking-widest block mb-1.5">Max Powerups (Optional)</label>
              <input type="number" value={maxPowerups ?? ''} onChange={e => setMaxPowerups(e.target.value ? parseInt(e.target.value) : null)}
                placeholder="Tournament default"
                className="w-full bg-black/40 border-2 border-white/10 py-2.5 px-4 text-white font-display text-sm focus:outline-none focus:border-ipl-gold transition-all" />
              <p className="text-gray-600 text-[10px] font-display mt-1">Override powerup limit for these matches.</p>
            </div>
          )}
        </div>
      </div>
