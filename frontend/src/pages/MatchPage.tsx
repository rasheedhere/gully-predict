...
export default function MatchPage() {
  ...
  const winnerQId = useMemo(() => {
    ...
    return questions.find((q: any) =>
      q.options && q.options.length >= 2 && Array.from(teamSet).every((team: string) => q.options.includes(team))
    )?.key || null;
  }, [questions, match]);
                ...
                <div className="flex flex-col gap-1.5 md:gap-4">
                  ...
                  <div className="flex items-center justify-between gap-1 md:gap-4 bg-black/40 border border-white/10 rounded-lg md:rounded-2xl p-0.5 md:p-2 select-none">
                    ...
                    <div className="shrink-0">
                      {isLocked ? (
                        <span className="bg-ipl-gold text-black text-[7px] md:text-[10px] font-display font-bold uppercase tracking-widest px-2 py-1 rounded-full shadow-[0_0_15px_rgba(255,215,0,0.3)] md:animate-pulse">
                          Live
                        </span>
                      ) : (
